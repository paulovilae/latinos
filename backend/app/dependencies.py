...
class DataStore:
    ...
    def create_formula(self, bot_id: int, payload: Dict, created_by: int) -> FormulaVersion:
        version = len([f for f in self.formulas.values() if f.bot_id == bot_id]) + 1
        formula = FormulaVersion(
            id=self._formula_id, bot_id=bot_id, version=version, payload=payload, created_by=created_by
        )
