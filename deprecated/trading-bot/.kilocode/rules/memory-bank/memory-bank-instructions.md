...
- `testing-methodology.md` - Complete testing approach and standards
...
All pending tasks for the project must be created and tracked in the `.kilocode/tasks` directory. Each task should have a clear status:

1. **Not Started**: Tasks that have been identified but implementation has not begun
2. **In Progress**: Tasks where implementation has started but is not complete
   - For tasks in progress, document what has been implemented and what is still pending
3. **Done**: Tasks that have been fully implemented and tested

When updating task status, provide detailed information about progress, challenges encountered, and any design decisions made. This creates a clear historical record of the project's development.

The main tasks file is located at `.kilocode/tasks/pending-tasks.md` and should be kept up-to-date as the project progresses.
...
When implementing new features or making significant changes to the codebase, I must follow the file organization guidelines specified in `file-organization.md`. Key principles include:

1. Group related files in dedicated directories when a feature requires more than 5-7 files
2. Keep the root directory clean with only essential files
3. Organize files into logical subdirectories (docs/, scripts/, testing/, etc.)
4. Include README.md files in each major directory
5. Use consistent naming conventions (kebab-case preferred)
6. Apply the "folder threshold" rule to create appropriate subdirectories

For detailed guidance, I should refer to `file-organization.md` when organizing files for new features or refactoring existing code.
...
When organizing project files, I MUST follow the file organization guidelines in `file-organization.md` to maintain a clean, navigable codebase. This is especially important for features that require multiple files, which should be organized in dedicated directories with appropriate subdirectories.